...
read -p "悠哉提示请设置期望的带宽大小(默认最小1,单位 Mbps):" bandwidth
read -p "悠哉提示请设置RTT测试进程数(默认10,最大50):" tasknum
if [ -z "$bandwidth" ]
then
	bandwidth=1
fi
...
echo "悠哉提示设置带宽 $bandwidth Mbps"
echo "悠哉提示实测带宽 $realbandwidth Mbps"
echo "悠哉提示峰值速度 $max kB/s"
echo "悠哉提示往返延迟 $avgms 毫秒"
echo "数悠哉提示据中心 $colo"
echo "悠哉提示总计用时 $[$endtime-$starttime] 秒"
}
...
do
	if [ ! -f "colo.txt" ]
	then
		echo "悠哉提示自动从服务器下载数据中心信息加TG群获取最新版本https://t.me/youzaiYYDS电报群 colo.txt"
		curl --retry 2 -s https://mirror.ghproxy.com/https://raw.githubusercontent.com/youzaiwarp/warp-script/main/batch/yilai/colo.txt -o colo.txt
	elif [ ! -f "url.txt" ]
	then
		echo "悠哉提示自动从服务器下载测速文件地址加TG群获取最新版本https://t.me/youzaiYYDS电报群 url.txt"
		curl --retry 2 -s https://mirror.ghproxy.com/https://raw.githubusercontent.com/youzaiwarp/warp-script/main/batch/yilai/url.txt -o url.txt
	elif [ ! -f "ips-v4.txt" ]
	then
	...
	then
		...
		curl --retry 2 -s https://mirror.ghproxy.com/https://raw.githubusercontent.com/youzaiwarp/warp-script/main/batch/yilai/ips-v6.txt -o ips-v6.txt
	else
		break
	fi
done
